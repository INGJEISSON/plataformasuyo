...
if(isset($_SESSION['cod_usuario'])){
  ...
  // Consulto los diagnósticos que no tomaron el servicio
                       $sql8="select det_repor_aseso.tom_serv from enc_procesadas, det_repor_aseso where $parametro  enc_procesadas.id_fasfield=det_repor_aseso.id_fasfield and enc_procesadas.tipo_encuesta=2 and det_repor_aseso.tom_serv='No'  and enc_procesadas.cod_estado=6 and enc_procesadas.fecha_filtro between '".$_POST['fecha_1']."' and '".$_POST['fecha_2']."' ";
                        $query8=pg_query($conexion, $sql8);
                        $tom_servno=pg_num_rows($query8);
                       // Consulto los diagnósticos que tomaron el servicio
                       
                        $sql9="select det_repor_aseso.tom_serv from enc_procesadas, det_repor_aseso where $parametro enc_procesadas.id_fasfield=det_repor_aseso.id_fasfield and enc_procesadas.tipo_encuesta=2 and det_repor_aseso.tom_serv='Si' and enc_procesadas.cod_estado=6 and enc_procesadas.fecha_filtro between '".$_POST['fecha_1']."' and '".$_POST['fecha_2']."' ";
                        $query9=pg_query($conexion, $sql9);
                        $tom_servsi=pg_num_rows($query9); 
                        
                         // Consulto los diagnósticos pendientes por venta
                        $sql10="select det_repor_aseso.tom_serv from enc_procesadas, det_repor_aseso where $parametro enc_procesadas.id_fasfield=det_repor_aseso.id_fasfield and enc_procesadas.tipo_encuesta=2 and det_repor_aseso.tom_serv='Pendiente de venta'  and enc_procesadas.cod_estado=6 and enc_procesadas.fecha_filtro between '".$_POST['fecha_1']."' and '".$_POST['fecha_2']."' ";
                        $query10=pg_query($conexion, $sql10);
                        $tom_servpendventa=pg_num_rows($query10); 
                        
                          // Consulto los diagnósticos pendientes por venta
                        $sql11="select det_repor_aseso.tom_serv from enc_procesadas, det_repor_aseso where $parametro enc_procesadas.id_fasfield=det_repor_aseso.id_fasfield and enc_procesadas.tipo_encuesta=2 and det_repor_aseso.tom_serv='No viable'  and enc_procesadas.cod_estado=6 and enc_procesadas.fecha_filtro between '".$_POST['fecha_1']."' and '".$_POST['fecha_2']."' ";
            ...
            if(isset($_POST['repor_pagos'])){  // GGenerar dashboard  para contabilidad...
  ...
  // Consulto los diagnósticos que no tomaron el servicio
                     ...
                     $gratuito=pg_num_rows($query124); 
                      ...
                      $entr_diag=pg_num_rows($query126); */
                          ...
                          $sql126="select enc_procesadas.id_fasfield, det_repor_aseso.resul_visita from enc_procesadas, det_repor_aseso where $parametro enc_procesadas.id_fasfield=det_repor_aseso.id_fasfield and enc_procesadas.tipo_encuesta=2 and (det_repor_aseso.resul_visita='".('Entrega de diagnÃ³stico')."' or det_repor_aseso.resul_visita='".('Entrega de diagnóstico')."' ) and enc_procesadas.fecha_filtro between '".$_POST['fecha_1']."' and '".$_POST['fecha_2']."' ";
                        $query126=pg_query($conexion, $sql126);
                      $entr_diag=pg_num_rows($query126); 
                        // Sumamos los servicios express..
       ...
       if(isset($_POST['add_revi_call'])){ // Agregar revisiónes (Seguimientos)
              ...
              if($query){
                        ...
                        if($_POST['tipo_seguimiento']<8 or $_POST['tipo_seguimiento']==19)
                         ...
                         $sql2="select seguimientos.fecha_registro, seguimientos.archivo, seguimientos.observacion, estado.descripcion as estado, estado.cod_estado, usuarios.nombre as usuario from seguimientos, usuarios, estado where seguimientos.cod_usuario=usuarios.cod_usuario and seguimientos.cod_estado=estado.cod_estado and seguimientos.id_fasfield='".$_POST['id_fasfield']."' and seguimientos.cod_usuario!=0 and seguimientos.tipo_seguimiento='".$_POST['tipo_seguimiento']."' order by seguimientos.id_segui_llam desc  ";
                            ...
                            if($_POST['tipo_seguimiento']==8) // Afectaciones
                             ...
                             else if($_POST['tipo_seguimiento']==14 or $_POST['tipo_seguimiento']==15 or $_POST['tipo_seguimiento']==16){ // Servicios (Ofrecidos, No Viables, Recomendables )

                                if($_POST['tipo_seguimiento']==14 or $_POST['tipo_seguimiento']==15 or $_POST['tipo_seguimiento']==16){

                                  $_POST['pr_servi_n']="";
                                      if($_POST['tipo_seguimiento']==14) // OFrecidos
                                        $tipo=1;
                                      if($_POST['tipo_seguimiento']==15) // REcomeendados
                                        $tipo=2; 
                                      if($_POST['tipo_seguimiento']==16) // NO viables.
                                        $tipo=3;
                                                     // Verificamos que el servicio aún no esté recomendado..
                                      ...
                                      $rows=pg_num_rows($query2);
                                          ...
                                          if($rows)
                                            echo "3"; // Servicio ya se encuentra ofrecido en el diagnóstico..
                      ...
                      // actualizamos el valor aprobado y su estado
                            ...
                            $query1=pg_query($conexion, $update1);  
                                  ...
                                  if($_POST['cod_estado']==6){        

                                     // Buscamos las encuestas que estén en el estado de aprobado
                                $s="select distinct enc_procesadas.id_fasfield from enc_procesadas, det_repor_aseso where enc_procesadas.id_fasfield=det_repor_aseso.id_fasfield and det_repor_aseso.tom_serv='Si' and enc_procesadas.cod_estado=6 and ( tipo_pago='Contado' or tipo_pago='Cuotas') and  enc_procesadas.id_fasfield='".$_POST['id_fasfield']."' ";
                                              ...
                                              $query4=pg_query($conexion, $sql4); 
                                                $rows4=pg_num_rows($query4);
                                                 ...
                                                 if($rows4==0){ // SI no está registrado entonces..
                                                            ...
                                                            $insert="insert into cliente (id_fasfield, cod_cliente, nombre, tipo_cliente, ciudad, barrio, direccion_predio, telefono_1) values('".$_POST['id_fasfield']."', '".$datos['id_cliente']."',  '".$datos['cliente']."', 1, '".$datos['ciudad']."', '".$datos['barrio']."', '', '".$datos['telefono']."') ";
                                                        ...
                                                        $md5_carp=md5($carpeta_cliente);                                          
                                                          ...
                                                          // Verificamos que no tenga documentación

                                                          $s="select * from documentacion where cod_cliente='".$_POST['id_cliente']."' ";
                                                          $q2=pg_query($conexion, $sql2); 
                                                          $r2=pg_num_rows($q2);
                                                              if($r2==0) {
                                                   ...
                                                   if($datos['tom_serv']=='Si'){ // Es un servicio nuevo 
                                                          ...
                                                          for($i=0;$i<count($separar);$i++){
                                                               ...
                                                               $cod_servicio=$separar[$i];
                                                                ...
                                                                if(is_numeric($cod_servicio)){ // Si es un código de servicio entonces...
                                                                 ...
                                                                 $sql31="select * from serv_cliente where cod_servicio='".$cod_servicio."' and cod_cliente='".$datos['id_cliente']."' ";
                                                                ...
                                                                $rows31=pg_num_rows($query31);
                                                                          ...
                                                                          if($rows31==0){ // EL servicio no se ha registrado enttonces... agréguelo..
                                                                             ...
                                                                             $insert2="insert into serv_cliente (asesor, cod_servicio, cod_estado, cod_cliente, valor, fecha_registro, cod_usuario, cod_acuer_pago, id_list_despleg) values('".$datos['asesor']."', '".$cod_servicio."', 23, '".$datos['id_cliente']."', '".$datos['valor']."', '".$fecha_registro."', 95, 1, 1)  ";
...
$insert5="insert into usuarios (email, nombre, apellidos, tipo_usuario, cod_estado) values('".$_POST['email']."', '".$_POST['nombre']."', '".$_POST['apellidos']."', '".$_POST['tipo_usuario']."', 1) ";
    ...
    if(isset($_POST['add_revi_serv'])){ // Agregar actividades del servicio.
     ...
     if(isset($_POST['add_revi_diag'])){ // Agregar actividades del servicio.
      ...
      if(isset($_POST['edit_revi_diag'])){ // Editar  activiades...
          ...
          if($_SESSION['cod_grupo']==6 or $_SESSION['cod_grupo']==1) // Coordiandor de operaciones.  o Super administrador
            ...
            $tipo_revision=3; // Asesor
              ...
              if($_POST['edit']){
                    $insert="update activ_diag set observacion='".$_POST['observacion']."', fecha_actividad='".$_POST['fecha_actividad']."', fecha_registro='".$fecha_registro."', cod_usu_respon='".$_SESSION['cod_usuario']."' where id_activi_diag='".$_POST['id_activi_diag']."' ";
